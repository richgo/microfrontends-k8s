...
async function executeFetch(context, fetch, results, _path, traceNode) {
    ...
    async function sendOperation(context, source, variables) {
        ...
        if (response.errors) {
            const errors = response.errors.map(error => downstreamServiceError(error, fetch.serviceName, source, variables));
    ...
    for (const selection of selections) {
        switch (selection.kind) {
            case graphql_1.Kind.FIELD:
                const responseName = QueryPlan_1.getResponseName(selection);
...
function downstreamServiceError(originalError, serviceName, query, variables) {
    ...
    return new graphql_1.GraphQLError(message, undefined, undefined, undefined, path, originalError, extensions);
}
